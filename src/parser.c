...
void load_convolutional_weights(layer l, FILE *fp, int index)
{
    ...
    if(index < 14){
        ...
        do_conversion_f_to_h(l.biases_half,           l.biases,           l.n );
        
        if (l.batch_normalize && (!l.dontloadscales)){
            do_conversion_f_to_h(l.scales_half,           l.scales,           l.n );
            do_conversion_f_to_h(l.rolling_mean_half,     l.rolling_mean,     l.n );
...
void load_variance_weights(network *net, int start, int cutoff){
    fprintf(stderr, "load sqrt_variance_weights to weight_variance.weights\n");
    FILE *fp = fopen("weight_variance.weights", "rb");
    if(!fp) file_error("weight_variance.weights");
    int i,j;
...
void load_weights_upto(network *net, char *filename, int start, int cutoff)
{
    ...
    for(i = start; i < net->n && i < cutoff; ++i){
        ...
        if(l.type == CONVOLUTIONAL || l.type == DECONVOLUTIONAL){
            load_convolutional_weights(l, fp, i);
        }
        if(l.type == CONNECTED){
        ...
        if(l.type == CRNN){
            load_convolutional_weights(*(l.input_layer), fp, i);
            load_convolutional_weights(*(l.self_layer), fp, i);
            load_convolutional_weights(*(l.output_layer), fp, i);
        }
        if(l.type == RNN){

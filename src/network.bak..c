...
void forward_network(network *netp)
{
	...
	//int sz_in = net.layers[2].w * net.layers[2].h * net.layers[2].c;
    printf("sz_in : %d\n",sz_in);
//	int sz_out = net.layers[0].out_w * net.layers[0].out_h * net.layers[0].out_c;
//    printf("sz_out : %d\n", sz_out);
	mo_in = clCreateMemobj(CL_MEM_READ_ONLY, sizeof(float) * sz_in, NULL);
	cl_memcpy_to_device(mo_in, net.input, sizeof(float) * net.layers[0].w *  net.layers[0].h *  net.layers[0].c);
    printf("-----cpu------\n"); for(i = 0; i<30 ; i++) printf("%f ", net.input[i]); printf("-------\n");
//	mo_out = clCreateMemobj(CL_MEM_READ_ONLY, sizeof(float) * sz_out, NULL);
	layer l;
	float *out;
	...
	while (i < 15) {
        if(i == 0){
            ...
            //---------------------------------
            forward_conv_max(l2, net, 1, &mo_in);
            float *out2 = (float *)malloc(sizeof(float)*l.out_w * l.out_h * l.out_c);
            cl_memcpy_from_device(out2, mo_in, sizeof(float) * l.out_w*l.out_h*l.out_c);
            for (int k = 0; k < 20; k++){
                    printf("%f ", out2[k]);
            }
            for(int j = 0; j < l.out_w * l.out_h * l.out_c; j++){
                if(l.output[j] != out2[j]){ printf("nooo!!!! %f, %f, at %d \n", l.output[j], out2[j], j); break;}
        ...
        if(i <= 10){
            ...
            l = net.layers[i];
            if(i == 10){
                forward_conv_max(l, net, 0, &mo_in);
            }
            else{
                forward_conv_max(l, net, 1, &mo_in);
            }
            elapsed_time = what_time_is_it_now() - time;
            ...
            out = l.output;
            cl_memcpy_from_device(out, mo_in, sizeof(float) * l.out_w*l.out_h*l.out_c);
            net.input = l.output;

            l = net.layers[i];
            forward_maxpool_layer(l, net);
            printf("net.layers[%d].w : %d\nnet.layers[%d].h:%d\nnet.layers[%d].c:%d\n",i, net.layers[i].w, i, net.layers[i].h, i, net.layers[i].c);
            cl_memcpy_to_device(mo_in, net.input, sizeof(float) * net.layers[i].w *  net.layers[i].h *  net.layers[i].c);
            i++;
        }
        else if(i==14){
            out = l.output;

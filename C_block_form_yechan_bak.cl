				   ...
				   __global const half* scales, __global const half* biases)
{
	...
	__local float localFilter[LOCAL_DEPTH][9*4];
	__local float4 localFilter2[LOCAL_DEPTH][9];
	/*
	union
	{
	    __local float localFilter[LOCAL_DEPTH][9*4];
...
*/
	...
	if(global_x == 0 && global_y == 0 && global_z == 0) printf("weight 0 in gpu: %f\n", vload_half(0, imageB));
	/**************** padding *********************/
	...
	for( i = 0; i< inputChannel; i++){
		...
		if(i%4 == 0){
			if(idx_1d < 9*LOCAL_DEPTH){
				uweights.localFilter2[chunk_num][idx_1d%9] = vload_half4((LOCAL_DEPTH*global_z+chunk_num)*9*inputChannel + i*9 + idx_1d%9*4, imageB);
			}
			barrier(CLK_LOCAL_MEM_FENCE);
		}
		*/
